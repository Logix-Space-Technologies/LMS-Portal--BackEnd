...
const CollegeStaffViewAttendance = () => {
    ...
    const apiUrl4 = global.config.urls.api.server + "/api/lms/colgstaffSearchattendance";
    ...
    const readSearchValue = () => {
        ...
        const data = { "sessionId": sessionStorage.getItem("viewattendanceid"), "attendanceSearchQuery": inputField.attendanceSearchQuery };
    ...
    return (
        <div>
            ...
            <div className="flex justify-between items-center mt-8 ml-4 mb-4">
                {key === 'lmsapp' ? <h2 className="text-lg font-bold">View Attendance Session-Wise ( Session Name {currentAttendances.length > 0 ? `- ${currentAttendances[0].sessionName}` : ''})</h2> : (key === 'lmsappclgstaff' ? <h2 className="text-lg font-bold">View Attendance Session-Wise ( Session Name {currentAttendances.length > 0 ? `- ${currentAttendances[0].sessionName}` : ''})</h2> : <h2 className="text-lg font-bold">View Attendance Session-Wise ( Session Name {currentAttendances.length > 0 ? `- ${currentAttendances[0].sessionName}` : ''})</h2>)}
