...
const callbackCheckClgStaff = (req, res) => {
    const data = req.body;
...
function handleEnqueuedMessage(payload) {
    ...
    db.query("UPDATE whatsappmsgfeedbackclgstaff SET queuedStatus = 1 WHERE msgId = ?", [payload.id], function(err) {
        if (err) {
...
function handleMessageReceived(data) {
   ...
   console.log("Before calling convertToMySQLTimestamp");
    ...
    db.query("SELECT * FROM `college_staff` WHERE `phNo` = ?", [phNo], (err, res) => {
        if (err) {
...
module.exports.sendfn = callbackCheckClgStaff;