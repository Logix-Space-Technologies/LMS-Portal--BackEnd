...
async function sendWhatsAppMessage(destinationPhoneNumber, name, batchName, originalDate, originalTime, rescheduledDate, rescheduledTime, venueLink, sessionType, studId) {
    try {
        const payload = {
            channel: 'whatsapp',
            source: '918301000082',
        ...
        db.query("INSERT INTO `whatsappmsgfeedback`(`studId`, `msgId`, message) VALUES (?,?,'Session Rescheduled')", [studId, response.data.messageId],
            (err, res) => {
                if (err) {
                    console.log(err)
                } else {
