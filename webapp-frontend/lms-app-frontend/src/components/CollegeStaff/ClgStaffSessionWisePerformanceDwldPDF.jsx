...
const ClgStaffSessionWisePerformanceDwldPDF = () => {
    ...
    const sessionId = sessionStorage.getItem("ViewsessionperformanceSessionId");
    ...
    const generatePDF = async () => {
        ...
        const opt = {
            margin: 0.4,
            filename: 'session_wise_performance_list.pdf',
        ...
        html2pdf().set(opt).from(element).toPdf().get('pdf').then(function (pdf) {
            ...
            pdf.save('session_wise_performance_list.pdf');
        });
    };

    const groupedData = {};
    scorePDFData.forEach(clgStaff => {
        if (!groupedData[clgStaff.sessionName]) {
            groupedData[clgStaff.sessionName] = [];
        }
        groupedData[clgStaff.sessionName].push(clgStaff);
    });
                            ...
                            <div className="bg-white p-4 p-md-5 rounded shadow-sm">
                                <div className="row gy-3 gy-md-4 overflow-hidden">
                                    <div className="flex justify-between items-center mx-4 my-4">
                                        ...
                                        <strong style={{ textAlign: "center", fontSize: "24px" }}>Download Preview Of Session-Wise Performance List</strong>
                                        <div>
                                            ...
                                            return (
                                                <div key={sessionName}>
                                                    ...
                                                    <table style={{ borderCollapse: 'collapse', width: '100%' }}>
                                                        ...
                                                        <tbody>
                                                            {groupedData[sessionName].map((clgStaff, index) => (
                                                                <tr key={index}>
                                                                    <td style={{ border: '1px solid black', padding: '8px' }}>{clgStaff.membership_no}</td>
                                                                    <td style={{ border: '1px solid black', padding: '8px' }}>{clgStaff.studName}</td>
                                                                    <td style={{ border: '1px solid black', padding: '8px' }}>{clgStaff.score.toFixed(2)}</td>
                                                                    <td style={{ border: '1px solid black', padding: '8px' }}>{clgStaff.totalScore}</td>
                                                                </tr>
                                                            ))}
                                                        </tbody>
                                                    </table><br />
...
export default ClgStaffSessionWisePerformanceDwldPDF