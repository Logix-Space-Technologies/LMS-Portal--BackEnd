...
Student.searchStudentsOfCollegeByBatchId = (searchKey, batchId, result) => {
    ...
    db.query(
        "SELECT c.collegeName, b.batchName, s.id, s.studName, s.admNo, s.rollNo, s.studDept, s.course, s.studEmail, s.studPhNo, s.studProfilePic, s.aadharNo, s.membership_no, s.validity FROM student s JOIN college c ON s.collegeId = c.id JOIN batches b ON s.batchId = b.id WHERE b.id = ? AND c.deleteStatus = 0 AND c.isActive = 1 AND s.deleteStatus = 0 AND s.isActive = 1 AND s.validity > CURRENT_DATE AND s.isVerified = 1 AND b.isActive = 1 AND b.deleteStatus = 0 AND (s.studName LIKE ? OR s.membership_no = ? OR s.rollNo = ? OR s.admNo = ? OR s.course LIKE ? OR s.studDept LIKE ?) ORDER BY s.membership_no ASC",
...
Student.viewBatch = (collegeId, result) => {
    db.query(
        "SELECT DISTINCT  b.batchName, b.regStartDate, b.id, b.regEndDate, b.batchDesc, b.batchAmount, b.addedDate, b.registrationStatus FROM batches b JOIN college c ON b.collegeId = c.id WHERE b.deleteStatus = 0 AND b.isActive = 1 AND c.deleteStatus = 0 AND c.isActive = 1 AND c.id = ?",
        [collegeId],
        (err, res) => {
            if (err) {
                console.log("error: ", err);
...
Student.studentNotificationViewUpdate = (studId, batchId, result) => {
    const getStudentQuery = "SELECT * FROM student WHERE id = ? AND deleteStatus = 0 AND isActive = 1 AND emailVerified = 1 AND isVerified = 1 AND isPaid = 1";
    const getBatchQuery = "SELECT * FROM batches WHERE id = ? AND deleteStatus = 0 AND isActive = 1";
    const getNotificationsQuery = `
        SELECT 
            ...
            notifications.sendDateTime, 
            CASE 
                WHEN TIMESTAMPDIFF(MINUTE, notifications.sendDateTime, NOW()) < 1 THEN 'now' 
                WHEN TIMESTAMPDIFF(MINUTE, notifications.sendDateTime, NOW()) < 60 THEN CONCAT(TIMESTAMPDIFF(MINUTE, notifications.sendDateTime, NOW()), 'm ago') 
                WHEN TIMESTAMPDIFF(HOUR, notifications.sendDateTime, NOW()) < 24 THEN CONCAT(TIMESTAMPDIFF(HOUR, notifications.sendDateTime, NOW()), 'h ago') 
                ELSE CONCAT(TIMESTAMPDIFF(DAY, notifications.sendDateTime, NOW()), 'd ago') 
            END AS formattedDateTime, 
            CASE 
                WHEN notifications.sendBy = 0 THEN 'Admin' 
                ELSE COALESCE(admin_staff.AdStaffName, 'Unknown') 
            END AS senderName 
        FROM notifications 
        LEFT JOIN admin_staff ON notifications.sendBy = admin_staff.id 
        WHERE notifications.batchId = ? 
        AND notifications.sendDateTime >= DATE_SUB(NOW(), INTERVAL 14 DAY) 
        ORDER BY notifications.sendDateTime DESC
    `;

    db.query(getStudentQuery, [studId], (err, studentRes) => {
        if (err) {
        ...
        if (studentRes.length === 0) {
            console.log("Student not found or not verified");
            return result("Student not found or not verified", null);
        }

        db.query(getBatchQuery, [batchId], (err, batchRes) => {
            if (err) {
            ...
            db.query(getNotificationsQuery, [batchId], (err, notificationsRes) => {
                if (err) {
...
Student.generateTaskWiseScoreList = (taskId, result) => {
    let query = "SELECT taskName, membership_no, studName, score, totalScore FROM studentTaskScore WHERE taskId = ? GROUP BY studName, membership_no, taskName, score, totalScore ORDER BY score DESC"
