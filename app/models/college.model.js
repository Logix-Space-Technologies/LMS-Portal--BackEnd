...
College.viewPerformanceOfStudents = (collegeId, result) => {
    db.query("SELECT batchName, membership_no, studName,sum(score) as score ,sum(totalScore) as totalScore FROM studentTaskScore where CollegeId=1 AND dueDate < CURRENT_DATE GROUP BY batchName,studentId,studName order by batchName, score DESC;",[collegeId],(err,res)=>{
        if(err){
            console.log("Error : ", err);
            result(err, null);
            return;
