                ...
                if (err) {
                    ...
                    db.query('SELECT s.*, b.batchName, c.collegeName FROM student s JOIN batches b ON s.batchId = b.id JOIN college c ON b.collegeId = c.id WHERE s.id = ?', [request.body.studId], (err, result) => {
                        if (err) {
                            ...
                            let addedDate = result[0].addedDate.toLocaleDateString('en-IN', { timeZone: 'Asia/Kolkata', day: '2-digit', month: '2-digit', year: 'numeric', hour: '2-digit', minute: '2-digit', second: '2-digit'  });
