...
Batches.getOverallPerformanceOfBatch = (CollegeId, batchId, result) => {
    db.query(`SELECT studentId, membership_no, studName,sum(score) as score ,sum(totalScore) as totalScore FROM studentTaskScore where CollegeId=? and batchId=? and dueDate < CURRENT_DATE GROUP BY studentId,studName order by score DESC`, [CollegeId, batchId], (err, res) => {
        if (err) {
            console.log("error: ", err);
            result(err, null);
            return;
