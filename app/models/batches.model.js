...
Batches.ClgStaffNotificationView = (id, result) => {
    db.query("SELECT * FROM batches WHERE id = ? AND deleteStatus = 0 AND isActive = 1", [id], (err, batchRes) => {
        if (err) {
            console.log("error: ", err);
            result(err, null);
            return;
...
Batches.getOverallPerformanceOfBatch = (CollegeId, batchId, result) => {
    db.query(`SELECT studentId, membership_no, studName,sum(score) as score ,sum(totalScore) as totalScore FROM studentTaskScore where CollegeId=? and batchId=? and dueDate < CURRENT_DATE GROUP BY studentId,studName order by studentId, score DESC`, [CollegeId, batchId], (err, res) => {
        if (err) {
            console.log("error: ", err);
            result(err, null);
            return;
