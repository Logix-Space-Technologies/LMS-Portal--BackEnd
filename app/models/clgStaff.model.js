...
CollegeStaff.viewTask = (sessionId, result) => {
    ...
    t.totalScore, 
    CASE 
        WHEN t.dueDate < CURRENT_DATE() THEN 'Past Due Date' 
        ELSE t.dueDate 
    END AS dueDate, 
    t.addedDate 
FROM 
    task t 
LEFT JOIN 
    sessiondetails s ON s.id = t.sessionId 
WHERE 
    t.deleteStatus = 0 
    AND t.isActive = 1 
    ...
    AND s.id = ? 
ORDER BY 
    dueDate DESC;`, [sessionId], (err, res) => {
        if (err) {
            console.log("error: ", err)
            result(err, null)
            return
