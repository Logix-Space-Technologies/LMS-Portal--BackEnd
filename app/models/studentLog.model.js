...
StudentLog.getAll = async (result) => {
    let query = "SELECT s.studName, stl.* FROM studentlogs stl JOIN student s ON stl.StudentId = s.id WHERE s.deleteStatus = 0 AND s.isActive = 1 AND stl.DateTime >= DATE_SUB(NOW(), INTERVAL 1 MONTH) ORDER BY stl.DateTime DESC"
